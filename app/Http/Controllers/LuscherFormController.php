...
class LuscherFormController extends Controller
{
    // Function retiverd data from `clien_data` table
    public function index()
    {
        $clients = LuscherForm::all();
    ...
    public function register(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'client_email' => 'required|email|unique:luscher_forms',
            'client_phone' => 'required|unique:luscher_forms'
        ]);
        ...
        if ($validator->fails()) {
            ...
            $registraions = LuscherForm::create($request->all());
            if(!$registraions){
                return response()->json([
            ...
            LuscherFormController::ERPLogin($request);
            // LuscherFormController::sendMessage($request);
            }
    ...
    {
        $client = LuscherForm::where('client_phone', $phone)->first();
        if (!$client) {
            // No data found for the given phone number
            LuscherFormController::getData($phone);
            return true;
        } else {
            return false;
        }
